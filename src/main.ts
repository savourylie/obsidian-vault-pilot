  ...
  display(): void {
    ...
    const showModelsWarning = (el: HTMLElement) => {
      ...
      if (isLM) {
        ...
        note.appendText(
          "Enable the Local Server (OpenAI-compatible) in LM Studio: "
        );
        note.createEl("a", {
          text: "LM Studio",
    ...
    const clearWarning = (el: HTMLElement) => {
      el.empty();
    };

    const setReloadButtonsLoading = (loading: boolean) => {
      if (chatReloadBtn && chatReloadBtn.setDisabled)
        chatReloadBtn.setDisabled(loading);
      if (editReloadBtn && editReloadBtn.setDisabled)
        editReloadBtn.setDisabled(loading);
      chatReloadBtn?.setTooltip?.(
        loading ? "Loading models…" : "Reload models"
      );
      editReloadBtn?.setTooltip?.(
        loading ? "Loading models…" : "Reload models"
      );
    };
    ...
    const tabs: Tabs = {
      chat: {
        ...
        content_generator: (container) => {
          ...
          chatSystemPromptTextarea.value =
            this.plugin.settings.chatSystemPrompt || "";
                ...
                .onChange(async (value) => {
                  ...
                  if (!isNaN(num) && num >= 1) {
                    ...
                    if (this.plugin.settings.reservedResponseTokens >= num) {
                      this.plugin.settings.reservedResponseTokens = Math.max(
                        1,
                        num - 1
                      );
                    }
        ...
        content_generator: (container) => {
          new Setting(container)
            ...
            .addToggle((toggle: any) => {
              toggle.setValue(
                this.plugin.settings.tagSuggestions?.useLLM !== false
              );
          ...
          new Setting(container)
            ...
            .addToggle((toggle: any) => {
              toggle.setValue(
                this.plugin.settings.tagSuggestions?.confirmBeforeInsert !==
                  false
              );
              toggle.onChange(async (value: boolean) => {
                this.plugin.settings.tagSuggestions = this.plugin.settings
          ...
          new Setting(container)
            ...
            .addText((text) =>
              text
                ...
                .setValue(
                  this.plugin.settings.tagSuggestions?.modelOverride ?? ""
                )
                .onChange(async (value) => {
                  this.plugin.settings.tagSuggestions = this.plugin.settings
                    .tagSuggestions || {
